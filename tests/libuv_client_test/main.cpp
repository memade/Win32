...
#define ENABLE_SERVER_UDP 0
#define ENABLE_CLIENT_UDP 1
...
int main(int argc, char** argv) {
...
#endif
 auto client_tcp_ = libuv::IClient::CreateInterface(
 ...
 auto config = client_tcp_->ConfigGet();
#if ENABLE_CLIENT_TCP || ENABLE_CLIENT_UDP
 config->IPAddrV4("127.0.0.1");
 config->Port(8888);
#endif
#if ENABLE_CLIENT_IPC
 ...
 shared::Win::MainProcess(
  [&](const std::string& input, bool& exit) {
   if (input == "q") {
    client_tcp_->Stop();
    libuv::IClient::DestoryInterface(client_tcp_);
    exit = true;
   }
  });
 return 0;
