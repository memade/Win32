...
namespace local {
 ...
 void Directx10Drive::CleanupRenderTarget() {
  if (m_mainRenderTargetView) {
   m_mainRenderTargetView->Release();
   m_mainRenderTargetView = NULL;
  }
 }
 void Directx10Drive::WaitForLastSubmittedFrame() {

 }
 void Directx10Drive::ResetDevice() {
 ...
 LRESULT WINAPI Directx10Drive::WindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  if (ImGui_ImplWin32_WndProcHandler(hWnd, msg, wParam, lParam))
   return true;
  static Directx10Drive* _pThis = nullptr;
  switch (msg) {
  case WM_CREATE: {
   LPCREATESTRUCTW pCreateStructW = reinterpret_cast<LPCREATESTRUCTW>(lParam);
   if (!pCreateStructW)
    break;
   _pThis = reinterpret_cast<Directx10Drive*>(pCreateStructW->lpCreateParams);
   if (!_pThis)
    break;
   auto sk = 0;
  }break;
